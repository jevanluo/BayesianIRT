...
# MCMC Chain Update
run.chain.2pl <- function(M.burnin, M.keep, M.thin, U.data, hyperpars,
        ...
        rownames(chain.keep) <- c(paste('theta.abl', 1:length(th.init)),
                                  paste('a.disc', 1:length(a.init)),
                                  paste('b.diff', 1:length(b.init)),
                                  'sigma.theta')
        
        # Burnin phase : do not keep these results
        if(M.burnin > 0){
